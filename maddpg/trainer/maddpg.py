...
def make_update_exp(vals, target_vals):
    ...
    for var, var_target in zip(
        sorted(vals, key=lambda v: v.name), sorted(target_vals, key=lambda v: v.name)
    ):
...
def p_train(
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        train = U.function(
            inputs=obs_ph_n + act_ph_n, outputs=loss, updates=[optimize_expr]
        )
        ...
        return (
            ...
            train,
            update_target_p,
            {"p_values": p_values, "target_act": target_act},
        )
